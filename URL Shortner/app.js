...
form.addEventListener("submit", async (e) => {
  ...
  if (!selectItem.value) {
    ...
    try {
      //call api
      const callApi = await fetch(`https://api.shrtco.de/v2/shorten?url=${selectItem.value}`);
      const getData = await callApi.json();
      const data = getData?.result.full_short_link2 ?? getData?.result.full_short_link;
